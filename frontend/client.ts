 ...
 */
import { calculateLayout as api_photo_calculate_calculateLayout } from "~backend/photo/calculate";
import { generateLayout as api_photo_generate_generateLayout } from "~backend/photo/generate";
import { removeBackground as api_photo_remove_background_removeBackground } from "~backend/photo/remove-background";
import { getSettings as api_photo_settings_getSettings } from "~backend/photo/settings";
...
export namespace photo {
    ...
    export class ServiceClient {
         ...
         */
        public async calculateLayout(params: RequestType<typeof api_photo_calculate_calculateLayout>): Promise<ResponseType<typeof api_photo_calculate_calculateLayout>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/photo/calculate`, {method: "POST", body: JSON.stringify(params)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_photo_calculate_calculateLayout>
        }

        /**
         * Generates a passport photo layout PDF
         */
        public async generateLayout(params: RequestType<typeof api_photo_generate_generateLayout>): Promise<ResponseType<typeof api_photo_generate_generateLayout>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/photo/generate`, {method: "POST", body: JSON.stringify(params)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_photo_generate_generateLayout>
        }

        /**
         * Gets available photo settings
         */
        public async getSettings(): Promise<ResponseType<typeof api_photo_settings_getSettings>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/photo/settings`, {method: "GET", body: undefined})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_photo_settings_getSettings>
        }

        /**
         * Removes background from image and applies new background color
         */
        public async removeBackground(params: RequestType<typeof api_photo_remove_background_removeBackground>): Promise<ResponseType<typeof api_photo_remove_background_removeBackground>> {
            // Now make the actual call to the API
